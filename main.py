...
if(sys.argv[2]=="regr"):
    ...
    mLTregr=np.zeros(3)
    mR2Vregr=np.zeros(3)
    mSEGKregr=np.zeros(3)
    mDRNEregr=np.zeros(3)
    mS2Vregr=np.zeros(3)
    mGASregr=np.zeros(3)
    mRidregr=np.zeros(3)
    t = 50
    for i in range(t):
        ...
        BDEregr[1] = BDEregr[1] + regr(redModelA,vector,cl)
        BDEregr[2] = BDEregr[2] + regr(redModelA,vector,bt)
        if(name!="film"):
            mLTregr[0] = mLTregr[0] + regr(mLT,vector,ei)
            mLTregr[1] = mLTregr[1] + regr(mLT,vector,cl)
            mLTregr[2] = mLTregr[2] + regr(mLT,vector,bt)
        if(name!="film"):
            mSEGKregr[0] = mSEGKregr[0] + regr(mSEGK,vector,ei)
            mSEGKregr[1] = mSEGKregr[1] + regr(mSEGK,vector,cl)
            mSEGKregr[2] = mSEGKregr[2] + regr(mSEGK,vector,bt)
        mDRNEregr[0] = mDRNEregr[0] + regr(mDRNE,vector,ei)                               
        mDRNEregr[1] = mDRNEregr[1] + regr(mDRNE,vector,cl)
        mDRNEregr[2] = mDRNEregr[2] + regr(mDRNE,vector,bt)
        if(name!="film" and name!="actor"):
            mS2Vregr[0] = mS2Vregr[0] + regr(mS2V,vector,ei)
            mS2Vregr[1] = mS2Vregr[1] + regr(mS2V,vector,cl)
            mS2Vregr[2] = mS2Vregr[2] + regr(mS2V,vector,bt)
        mGASregr[0] = mGASregr[0] + regr(mGAS,vector,ei)
        mGASregr[1] = mGASregr[1] + regr(mGAS,vector,cl)
        mGASregr[2] = mGASregr[2] + regr(mGAS,vector,bt)
        mRidregr[0] = mRidregr[0] + regr(mRid,vector,ei)
        mRidregr[1] = mRidregr[1] + regr(mRid,vector,cl)
        mRidregr[2] = mRidregr[2] + regr(mRid,vector,bt)
        

    f = open("results/"+name+"Regression.csv","w")
    f.write("Method;Eigen;Close;Between\n")

    f.write("BDEEmb;"+str(BDEregr[0]/t).replace(".",",")+";"+str(BDEregr[1]/t).replace(".",",")+";"+str(BDEregr[2]/t).replace(".",",")+"\n")
    f.write("GWEmb;"+str(mLTregr[0]/t).replace(".",",")+";"+str(mLTregr[1]/t).replace(".",",")+";"+str(mLTregr[2]/t).replace(".",",")+"\n")
    f.write("SEGKEmb;"+str(mSEGKregr[0]/t).replace(".",",")+";"+str(mSEGKregr[1]/t).replace(".",",")+";"+str(mSEGKregr[2]/t).replace(".",",")+"\n")
    f.write("DRNEEmb;"+str(mDRNEregr[0]/t).replace(".",",")+";"+str(mDRNEregr[1]/t).replace(".",",")+";"+str(mDRNEregr[2]/t).replace(".",",")+"\n")
    f.write("S2VEmb;"+str(mS2Vregr[0]/t).replace(".",",")+";"+str(mS2Vregr[1]/t).replace(".",",")+";"+str(mS2Vregr[2]/t).replace(".",",")+"\n")
    f.write("GASEmb;"+str(mGASregr[0]/t).replace(".",",")+";"+str(mGASregr[1]/t).replace(".",",")+";"+str(mGASregr[2]/t).replace(".",",")+"\n")
    f.write("RidEmb;"+str(mRidregr[0]/t).replace(".",",")+";"+str(mRidregr[1]/t).replace(".",",")+";"+str(mRidregr[2]/t).replace(".",",")+"\n")
